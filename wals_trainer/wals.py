...
def get_rmse(output_row, output_col, actual):
  ...
  rmse = math.sqrt(mse)
  # rmse = np.square(actual - output_row.dot(output_col.transpose())).mean()
  return rmse


def simple_train(model, input_tensor, num_iterations, tr_sparse, test_sparse):
  ...
  config_.intra_op_parallelism_threads = 6
  config_.inter_op_parallelism_threads = 6
  sess = tf.Session(graph=input_tensor.graph, config=config_)
  ...
  with input_tensor.graph.as_default():
    ...
    for ITER in xrange(num_iterations):
      # print ("ITERATTION {} STARTED".format(ITER))
      # for D in range(32):
      #   with tf.device('/device:CPU:{}'.format(D)):
      # print ("Updating row factors")
      sess.run(model.row_update_prep_gramian_op)
      sess.run(model.initialize_row_update_op)
      sess.run(row_update_op)
  # for d in ['/device:GPU:0', '/device:GPU:1']:
  #   with tf.device(d):
  #     print ("Updating col factors")
  ...
  #     infer = tf.tensordot(model.row_factors[0].eval(session=sess), model.col_factors[0].eval(session=sess).T, 1)
  #     train_ip = tf.sparse_to_dense(sparse_indices=zip(tr_sparse.row, tr_sparse.col),
  #                                   output_shape=tr_sparse.shape,
  #                                   sparse_values=(tr_sparse.data).astype(np.float32))
  #     # train_ip = tr_sparse.toarray()
  #     # train_ip = tf.sparse_to_dense(input_tensor)
  #     train_rmse = sess.run(tf.metrics.root_mean_squared_error(train_ip, infer))
  #     test_ip = tf.sparse_to_dense(sparse_indices=zip(test_sparse.row, test_sparse.col),
  #                        output_shape=test_sparse.shape,
  #                        sparse_values=(test_sparse.data).astype(np.float32))
  #     test_rmse = sess.run(tf.metrics.root_mean_squared_error(test_ip, infer))
  #     #
  #     #
  #     tf.logging.info('ITER{}: train RMSE = {}'.format(ITER, train_rmse))
  #     print('ITER{}: train RMSE = {}'.format(ITER, train_rmse))
  #     tf.logging.info('ITER{}: test RMSE = {}'.format(ITER, test_rmse))
  #     print('ITER{}: test RMSE = {}'.format(ITER, test_rmse))
  #     print ("ITERATION {} COMPLETED".format(ITER))
    print ("training ends")
  return sess
