...
def create_test_and_train_sets(args, input_file, data_type='ratings', valid=False):
  ...
  if data_type == 'ratings':
    ...
    print ("Preparing implicit data")
    return _page_views_train_and_test(input_file, args, valid=valid)
  else:
...
def _page_views_train_and_test(input_file, args, valid=False):
  ...
  # pds_items = np.load(os.path.join(os.getcwd(), model_dir, 'item.npy'))
  # pds_users = np.load(os.path.join(os.getcwd(), model_dir, 'user.npy'))
  # pv_ratings = np.load(os.path.join(os.getcwd(), model_dir, 'ratings.npy'))
  # create train and test sets
  if not valid:
      # tr_sparse, test_sparse = create_sparse_train_and_test(pv_ratings,
      #                                                       pds_users.size,
      #                                                       pds_items.size)
      tr_sparse = create_sparse_train_and_test(pv_ratings,
                                                pds_users.size,
                                                pds_items.size)
      print ("Data ready to train")

      # return pds_users, pds_items, tr_sparse, test_sparse
      return pds_users, pds_items, tr_sparse
  else:
      return pds_users, pds_items, pv_ratings
...
def generate_sparse_train_and_test(ratings, n_users, n_items, test_idx):
  """Given ratings, create sparse matrices for train and test sets.
...
def train_model(args, tr_sparse, test_sparse):
  ...
  # factorize matrix
  session = wals.simple_train(model, input_tensor, num_iters, tr_sparse, test_sparse)
...
def save_model(args, user_map, item_map, row_factor, col_factor):
  ...
  if not os.path.exists(model_dir):
    os.makedirs(model_dir)
  np.save(os.path.join(model_dir, 'user_new'), user_map)
  np.save(os.path.join(model_dir, 'item_new'), item_map)
  np.save(os.path.join(model_dir, 'row_new'), row_factor)
  np.save(os.path.join(model_dir, 'col_new'), col_factor)
